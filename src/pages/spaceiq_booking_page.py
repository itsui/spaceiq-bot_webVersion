...
class SpaceIQBookingPage(BasePage):
    ...
    def load_desk_positions(self) -> Dict[str, Tuple[int, int]]:
        """
        Load desk positions from the position cache file.

        Returns:
            Dictionary mapping desk codes to (x, y) coordinates
        """
        try:
            # Get the project root directory
            current_dir = Path(__file__).parent
            project_root = current_dir.parent.parent
            positions_file = project_root / "config" / "desk_positions.json"

            if not positions_file.exists():
                print(f"⚠️  Desk positions file not found: {positions_file}")
                return {}

            with open(positions_file, 'r', encoding='utf-8') as f:
                data = json.load(f)

            # Extract only the desk positions
            desk_positions = data.get('desk_positions', {})

            # Convert to the expected format (desk_id: (x, y))
            result = {}
            for desk_id, coords in desk_positions.items():
                if isinstance(coords, dict) and 'x' in coords and 'y' in coords:
                    result[desk_id] = (coords['x'], coords['y'])

            print(f"✅ Loaded {len(result)} desk positions from cache")
            return result
    ...
    async def get_available_desks_from_sidebar(self, desk_prefix: str, logger=None) -> list:
        ...
        # Only log count, not full list (reduces log spam)
        if logger:
            logger.info(f"Found {len(available_desks)} available desks")
    ...
    async def find_and_click_available_desks(self, available_desks: List[str], logger=None) -> Optional[str]:
        ...
        screenshot_files = sorted(
            self.screenshots_dir.glob("floor_map_loaded_*.png"),
        ...
        for desk_code in available_desks:
            if desk_code in desk_to_coords:
                ...
                # Found priority match
                if logger:
                    logger.info(f"Found priority desk {desk_code} (position {priority_pos})")
    ...
    async def click_available_desk_on_map(self, desk_preference: Optional[str] = None, desk_prefix: Optional[str] = None, logger=None):
        ...
        if desk_prefix:
            ...
            if len(available_desks) == 0:
                ...
                if logger:
                    logger.warning(msg)
                await self.capture_screenshot("no_available_desks", force=True)
                return False

            # Load desk coordinates from position cache
            desk_coordinates = self.load_desk_positions()

            if not desk_coordinates:
                msg = "❌ No desk positions loaded - cannot book desks"
                print(f"   {msg}")
                if logger:
                    logger.error(msg)
                await self.capture_screenshot("no_desk_positions", force=True)
                return False

            # Filter to only include desks we're looking for (2.24.* in this case)
            filtered_coordinates = {
                desk_id: coords for desk_id, coords in desk_coordinates.items()
                if desk_id.startswith(desk_prefix)
            }

            msg = f"Loaded {len(filtered_coordinates)} desk positions for prefix {desk_prefix}"
            print(f"   {msg}")
            if logger:
                logger.info(msg)
            ...
            for desk_id in available_desks:
                try:
                    ...
                    if desk_id in filtered_coordinates:
                        x, y = filtered_coordinates[desk_id]
            ...
            if logger:
                logger.warning(msg)
            await self.capture_screenshot("no_desks_clicked", force=True)
            return False
    ...
    async def verify_booking_success(self) -> bool:
        ...
        await self.capture_screenshot("booking_result", force=True)
    ...
    async def get_existing_bookings(self, logger=None) -> List[str]:
        ...
        try:
            ...
            # Also check "Past Bookings" tab for recent dates (today/yesterday)
            # This prevents rebooking when user has already checked in
                ...
                try:
                    ...
                    try:
                        ...
                        if date_str not in existing_bookings:
                            ...
                            # Only log recent past bookings (yesterday/today) to reduce noise
                            if logger and booking_date >= yesterday:
            ...
            if logger:
                # Filter to show only relevant dates (yesterday onwards) in logs to reduce noise
                relevant_bookings = [
                    d for d in existing_bookings
                    if datetime.strptime(d, '%Y-%m-%d').date() >= yesterday
                ]
                if relevant_bookings:
                    logger.info(f"Found {len(relevant_bookings)} existing bookings (in bookable range): {relevant_bookings}")
                else:
                    logger.info(f"Found {len(existing_bookings)} existing bookings (all in past)")
