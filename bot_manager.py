    ...
    async def _run_bot(self):
        ...
        with self.app_context:
            ...
            try:
                ...
                os.close(fd)  # Close if writing failed
                raise
        ...
        try:
            ...
            try:
                if 'session_file' in locals() and session_file.exists():
                    session_file.unlink()
                    logger.debug(f"Cleaned up temporary session file for user {self.user_id}")
            except Exception as e:
